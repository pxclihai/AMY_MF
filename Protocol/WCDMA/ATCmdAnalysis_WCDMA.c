...
{
	{0,STRING_NO_SERVICE,"uninet"},
	{1,"GSM","uninet"},
	{2,"GPRS","uninet"},
	{3,"EGPRS","uninet"},
	{4,"WCDMA","3gnet"},
	{5,STRING_ONLY"HSDPA","3gnet"},
	{6,STRING_ONLY"HSUPA","3gnet"},
	{7,"HSPA","3gnet"}
};
...
{AT_CMD_SET_CIPOPEN,    {"AT+CIPOPEN="},	                {"Connect ok","+IP ERROR: Connection is already created"},{"Connect fail"},	5000,	       	1,			ATCMDRECV_NAME(CIPOPEN)    ,true},	//建立一路TCP/IP 连接 
...
ATCMDGROUP_FUN(Init)
{
	...
	switch(_gModuleSetVal[0].Cns_CNMP)
	{
		case NETWORK_PRIORIYY_TYPE_1G_2G:	//1,2G优先
			 ...
			 //apnBuffer= "\"3gwap\"";
			//apnBuffer="\"uninet\"";
			sprintf(apnBuffer,"=1,\"IP\",\"%s\"",_gCnsModSet[3].apn);
			 break;
		case NETWORK_PRIORIYY_TYPE_2G_3G:	//2,3G优先
			netPriBuf = "14";
			//apnBuffer = "\"3gnet\"";
			sprintf(apnBuffer,"=1,\"IP\",\"%s\"",_gCnsModSet[5].apn);
			break;
...
ATCMDGROUP_FUN(DoLoop)
{
	...
	if(_gATCmdSetInfo.atCmdRunStatus ==  ATCMD_RUN_STATUS_RUN_FREE)
	{
		...
		if(_gATCmdSetInfo.apnStatus < 0)
		{
			int16 modeIndex =  _gModeRunState.CnsMod > (CNSMODSET_SIZE-1)? (CNSMODSET_SIZE-1):_gModeRunState.CnsMod;
			modeIndex = modeIndex < 0?0:modeIndex;
			//不同就设置APN
			sprintf(apnBuffer,"=1,\"IP\",\"%s\"",_gCnsModSet[modeIndex].apn);
			_gATCmdSetInfo.apnStatus = -1;
		}
		else if(_gATCmdSetInfo.apnStatus == 0)
		{
			sprintf(apnBuffer,"?");
			//查询
		} else if(_gATCmdSetInfo.apnStatus > 9)
		{
			_gATCmdSetInfo.apnStatus  = -1;
		}
