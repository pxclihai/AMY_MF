...
{AT_CMD_SET_CSMP,       {"AT+CSMP=4098,0,\"11/04/22,16:21:00\",1,12"},         	{"OK",""},		{""},    1000,	       	5,			NULL    		,true},	//设置TEXT模式参数，最后一个为数据编码类型，0表示默认字符集(GSM) 241:字符 25:中文
...
{AT_CMD_SET_NETOPEN,    {"AT+NETOPEN"},               	{"+NETOPEN: 0",""},{"ERROR"},		    30000,	       	2,			NULL    ,true},	//多链接方式
...
{AT_CMD_SET_TCPWDATA,   {""},	                         	{"+CIPSEND:",""},	{"ERROR"},	10000,	       	1,			NULL    	,true},	//在一路TCP/IP 下发送数据(数据体) 
...
{AT_CMD_SET_CIPCCFG,    {"AT+CIPCCFG=3,500,1,1,1,0,500"},	     	{"OK",""},      {""},   1000,	       	5,			NULL    		,true},  //配置参数的套接字
...
ATCMDGROUP_FUN(Init)
{
	...
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CIPSENDMODE,"=1",2);
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CIPMODE,"=0",2);

//	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CFUN,"",0);  //设置运行模式
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CMGF,"1",1);  //设置短信格式 
	//支持中文		
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CSMP,"8",1);  //设置TEXT模式参数
