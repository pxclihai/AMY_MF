...
{AT_CMD_SET_CSMP,       {"AT+CSMP=4098,0,\"15/07/10,12:09:00\",1,12"},         	{"OK",""},		{""},    1000,	       	5,			NULL    		,true},	//设置TEXT模式参数，最后一个为数据编码类型，0表示默认字符集(GSM) 241:字符 25:中文
...
//{AT_CMD_SET_CGSOCKCONT, {"AT+CGSOCKCONT"},      				{"OK","+CGSOCKCONT:"},		{"ERROR"},   1000,	       	1,			ATCMDRECV_NAME(CGSOCKCONT)    		,true},	//设置APN 
{AT_CMD_SET_NETOPEN,    {"AT+NETOPEN=,,1"},               	{"+NETOPEN: 0","Network opened"},{"ERROR"},		    30000,	       	2,			NULL    ,true},	//多链接方式
{AT_CMD_SET_CIPOPEN,    {"AT+CIPOPEN="},	                {"Connect ok","+IP ERROR: Connection is already created"},{"Connect fail"},	2000,	       	1,			ATCMDRECV_NAME(CIPOPEN)    ,true},	//建立一路TCP/IP 连接 
{AT_CMD_SET_CIPSEND,    {"AT+CIPSEND="},	                {">",""},		  {""},  2500,	       	1,			NULL    		,true},	//在一路TCP/IP 下发送数据(长度) 
{AT_CMD_SET_TCPWDATA,   {""},	                         	{"Send ok",""},	{"ERROR"},	10000,	       	1,			NULL    	,true},	//在一路TCP/IP 下发送数据(数据体) 
...
{AT_CMD_GET_CGREG,		{"AT+CGREG"},				{"+CGREG: 1","OK"},{"+CGREG: 0"},	10000,	1,		NULL,			true},	//获取GPRS状态
...
ATCMDGROUP_FUN(Init)
{
	...
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CFUN,"",0);		//******新添运行模式
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CNMP,netPriBuf,ReadStrLen(netPriBuf));	//设置上网优先选择模式


//	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CFUN,"",0);  //设置运行模式
...
//AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CSMP,"",0);  //设置TEXT模式参数
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CSCS,"\"UCS2\"",6);  //选择TE字符集GSM、UCS2、IRA
 	...
 	AtCmdAnalysis_AtCmd_Send(AT_CMD_STE_CPMS,"",0);  //短信存储位置选择命令*******修改了地址
	AtCmdAnalysis_AtCmd_Send(AT_CMD_GET_CGREG,"=1",2);	//GPRS
	...
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CIPCCFG,"",0);	//******新添套接字参数
	sprintf(apnBuffer,"=,,\"%s\",\"%s\"",_gCnsModSet[0].apn,_gCnsModSet[0].apn);
	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CSOCKAUTH,apnBuffer,ReadStrLen(apnBuffer));
//	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CMGD,"",0);
//	AtCmdAnalysis_AtCmd_Send(AT_CMD_SET_CSOCKSETPN,"1",1);
//	AtCmdAnalysis_AtCmd_Send(AT_CMD_GET_CGATT,"",0);	//GPRS	
