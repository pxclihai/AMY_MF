...
#include "Protocol.h"
#if CONF_QWL_NET_EN	
#define USART_FUNCTION_TYPE	 USART_FUNCTION_MSG		/*串口功能类型*/
...
chr = ReceiveDataFromUartStruct(_gUartStruct);
			API_net_rec(chr);	
	if(_gProtocol.Port_Type==PORT_TYPE_MSG) //端口为串口方式
	{
	...
	{ 
		if(_gProtocol.ResState==RESOURCE_USE_STATUS_FREE)
		{
			_gProtocol.Port_Type=PORT_TYPE_MSG;
			chr = ReceiveDataFromUartStruct(_gUartStruct);
			DoProtocolReceiveAnalysis(&_gProtocol,chr);
		}
		else
