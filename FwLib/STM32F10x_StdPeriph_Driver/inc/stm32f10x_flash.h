...
#define FLASH_BOOT_BanMSG  ((uint16_t)0x0001) /*!< At startup, if boot pins are set in boot from user Flash position
                                                  and this parameter is selected the device will boot from Bank 2 or Bank 1,
                                                  depending on the activation of the bank */
#define IS_FLASH_BOOT(BOOT) (((BOOT) == FLASH_BOOT_Bank1) || ((BOOT) == FLASH_BOOT_BanMSG))
#endif
/**
  * @}
  */
/** @defgroup FLASH_Interrupts 
  ...
  */
#ifdef STM32F10X_XL
#define FLASH_IT_BANMSG_ERROR                 ((uint32_t)0x80000400)  /*!< FPEC BANMSG error interrupt source */
#define FLASH_IT_BANMSG_EOP                   ((uint32_t)0x80001000)  /*!< End of FLASH BANMSG Operation Interrupt source */
...
/** @defgroup FLASH_Flags 
  ...
  */
#ifdef STM32F10X_XL
#define FLASH_FLAG_BANMSG_BSY                 ((uint32_t)0x80000001)  /*!< FLASH BANMSG Busy flag */
#define FLASH_FLAG_BANMSG_EOP                 ((uint32_t)0x80000020)  /*!< FLASH BANMSG End of Operation flag */
#define FLASH_FLAG_BANMSG_PGERR               ((uint32_t)0x80000004)  /*!< FLASH BANMSG Program error flag */
#define FLASH_FLAG_BANMSG_WRPRTERR            ((uint32_t)0x80000010)  /*!< FLASH BANMSG Write protected error flag */
...
#define IS_FLASH_GET_FLAG(FLAG)  (((FLAG) == FLASH_FLAG_BSY) || ((FLAG) == FLASH_FLAG_EOP) || \
                                  ...
                                  ((FLAG) == FLASH_FLAG_BANMSG_BSY) || ((FLAG) == FLASH_FLAG_BANMSG_EOP) || \
                                  ((FLAG) == FLASH_FLAG_BANMSG_PGERR) || ((FLAG) == FLASH_FLAG_BANMSG_WRPRTERR))
#else
#define FLASH_FLAG_BSY                 ((uint32_t)0x00000001)  /*!< FLASH Busy flag */
#define FLASH_FLAG_EOP                 ((uint32_t)0x00000020)  /*!< FLASH End of Operation flag */
#define FLASH_FLAG_PGERR               ((uint32_t)0x00000004)  /*!< FLASH Program error flag */
...
void FLASH_UnlockBanMSG(void);
void FLASH_LockBanMSG(void);
FLASH_Status FLASH_EraseAllBanMSGPages(void);
FLASH_Status FLASH_GetBanMSGStatus(void);
FLASH_Status FLASH_WaitForLastBanMSGOperation(uint32_t Timeout);
